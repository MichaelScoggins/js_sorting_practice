...
const stringsToNumbs = (numbers => numbers.map(x => parseInt(x)))
console.log(stringsToNumbs(strNums))

// With the same numbers, find the sum of the even values
const sumEvens = (numbers => numbers.map(x => parseInt(x)).filter(x => (x % 2 == 0)).reduce((x, y) => x + y))
...
atxIdx(strNums)
console.log(`index: ${atxIdx(strNums)}, value: ${strNums[505]}`)
...
//or this way
const weatherStates1 = () => {
  let newArray = []
  let weatherIterator = weather.values()
  for (x of weatherIterator) {
    newArray.push(x.weather_state_name)
  }
  return newArray.filter((x, y) => newArray.indexOf(x) == y)
}

console.log(weatherStates1())
//or this way
const weatherStates2 = () => {
  let newArray = []
  for (let i = 0; i != weather.length; i++) {
    newArray.push(weather[i].weather_state_name)
  }
  return newArray.filter((x, y) => newArray.indexOf(x) == y)
}

console.log("Weather States", weatherStates2())
